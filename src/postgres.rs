...
///
/// [`crate::redis::CursorStore`] deliberately has no equivalent: it is only used by the one-shot
/// `check` run, which exits long before an idle connection can be dropped underneath it.
